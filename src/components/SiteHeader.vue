<template>
  <header class="flex items-center justify-between py-2 px-5 shadow-slate-50">
    ...
    <div class="flex">
      <p class="mr-10 w-full max-w-36 text-sm text-right">
        <a class="block text-nowrap" :href="`tel:${appConfig.phone.replace(/[\s()]/g, '')}`" title="phone" target="_blank">{{ appConfig.phone }}</a>
      </p>

      <LanguageSwitcher />

      <div @click="toggleBurger" class="cursor-pointer ml-2 lg:hidden focus:outline-none">
      ...
      <svg @click="toggleBurger" v-if="isOpenMenu" class="cursor-pointer w-6 h-6 fixed top-6 right-6 z-50" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
